...
- [x] 🖥 **Webpage → PDF** (`/webpage-to-pdf` — shipped 2026-08-27, 3/day free → Pro unlimited). Google Chrome stable installed from Google's apt repo (Ubuntu 24.04 only ships a snap shim) + `puppeteer-core`.
  - **Chrome does NOT run as root.** The backend is root under pm2 and Chrome disables its own sandbox as root; `--no-sandbox` was rejected as the wrong trade for the one feature that loads hostile pages. It runs as unprivileged **`ddrender`** via `setpriv`, sandbox intact, over the DevTools socket on loopback. Verified on prod: processes owned by ddrender, zygotes present, port bound to 127.0.0.1, profile `0700`.
  - **SSRF hardening** in `backend/src/utils/ssrfGuard.js`: scheme allowlist, no credentials in URL, our own DNS resolve rejecting if ANY answer is private, and a re-check of **every request Chrome makes** (this is what closes redirects + DNS rebinding). Covers decimal/hex/octal IPv4, `::ffff:` mapped, 6to4, NAT64, CGNAT, metadata `169.254.169.254`. **58 assertions** in `dev-harness/ssrf-check.mjs`.
  - ⚠ Two gotchas found on prod, both fixed: `setpriv` does not change `HOME` (Chrome inherited `/root` and failed on startup), and the pm2 **cluster of two** fought over one Chrome profile (exit 21) — each instance now gets its own profile dir.
  - Roadmap said "Pro"; shipped on the same 3/day-free rule as every other server conversion for consistency. One line to change.
...
- [x] 📱 **Edit PDF details** (`/edit-pdf-metadata` — shipped 2026-08-26, free, on-device). Sets title/author/subject/keywords/creator/producer/dates, the sibling to Remove metadata. **The thing that makes it actually work:** a PDF can hold the same facts in both the Info dictionary and an XMP packet, and Acrobat believes XMP — so tools that write only Info appear to work and change nothing. We write both (stripping only the properties we own, appending one authoritative `rdf:Description`), and flag files that disagree with themselves. Batch mode stamps one author across many files; empty box = leave alone. Verified: 19 assertions in `dev-harness/docinfo-check.mjs` + a browser round trip.
- [x] 🖥 **PDF → RTF** (`/pdf-to-rtf`) and **PDF → ODT** (`/pdf-to-odt`) — shipped 2026-08-26, server, same Writer PDF import as PDF→Word, 3/day free → Pro. Verified on prod: valid RTF (398 KB) and ODT carrying 167 real words.
- [x] 🖥 **OpenDocument → PDF** (`/odf-to-pdf` — shipped 2026-08-26). ODT/ODS/ODP/**ODG** (+ flat-XML) in ONE page rather than four thin ones. Only ODG needed adding to `OFFICE_RE`. Verified through the live API: ODG and ODS both → valid PDF.
- [x] 📱 **PDF → HTML** (`/pdf-to-html` — shipped 2026-08-26, free, **on-device**). ⚠ Shipped server-side first and **immediately rebuilt**: LibreOffice's PDF import makes text into draw objects and its HTML filter can only export those as images — a real document came out as **17 words and 75 GIFs** (the XHTML filter produces nothing; via-ODT lands in the same place). Now runs over the same pdf.js text pipeline as PDF→Markdown: **161 words of real text, correct `<h1>`, 8 paragraphs, 4 lists**, one self-contained file, no upload, no daily cap. Images are not carried over and the page says so.
...
- [x] 📱 **Add bookmarks / outline editor** (`/add-bookmarks-to-pdf` — shipped 2026-08-27, free, on-device). Read/edit/add/nest/reorder bookmarks with undo, plus **build the whole outline from the document’s own headings in one click** (reuses the PDF→Markdown heading detection, so the two never disagree). pdf.js reads, pdf-lib has no outline API so the object tree is hand-built. Deleting a parent promotes its children; a page past the end is clamped; saving empty removes the outline AND the `/PageMode` that would open an empty panel. Verified: 17 assertions in `dev-harness/outline-check.mjs` walking the linked list as a reader does + 12 unit tests over the tree edits.
- [ ] ⏳ Repair PDF
...
## 3b. Receipt Scanner — full read ⭐ (2026-08-29, gated `coming_soon` / owner-only)

Reads the *whole* receipt, not just the total: every line item in printed order, each tax line under its printed label, subtotal, discounts, and every reference code — receipt/invoice/order no., REF#, TERMINAL/TE#, TC#, ST#, OP#, AUTH/APPR, store no. The identifier rule is deliberately store-agnostic and tells the reader to sweep the dense block **below the total and above the barcode**, keeping labels exactly as printed, including abbreviations it does not recognise — a chain-specific list would only ever work for the chains on it. Result panel shows a line-item table, an arithmetic check (lines → subtotal, subtotal + tax − discounts → total), the references, and a CSV export built in the browser.

**The failure this was built around, and the fix.** A real Walmart slip came back as an entirely different receipt — five identical apple juices, a beef chuck roast, subtotal 43.42 / total 47.28 against an actual 34.42 / 35.11, and a date eighteen months out. The cause was ordering: `autoRotate()` existed but ran only in the OCR fallback, *after* the vision attempt, so the model was handed the sideways photograph. A reader that cannot make out a receipt does not return nothing — it returns a plausible receipt, and because the invented figures were internally consistent the arithmetic check passed and put a **green tick on fabricated data**. That is worse than a blank result and worse than an error.

Three changes, in order of how much they buy:
1. **Straighten before reading, not after** — `autoRotate()` now runs before `readWithVision()`, and the OCR fallback reuses the already-upright file instead of rotating twice.
2. **Let it refuse** — the prompt now carries an `unreadable` flag and an explicit instruction that returning nothing is the correct answer to an unreadable image; the route believes it and falls through to OCR rather than presenting a guess as a reading.
3. **Show the photo first** — a `preview` phase between picking and scanning: the image, auto-turned upright (landscape ⇒ almost certainly a till roll on its side), manual **Left/Right**, a warning while it still reads sideways, and a **Start scanning** button. The rotation is baked into the bytes before upload, so what gets read is what you saw. Orientation rule lives in `lib/receipt-orientation.ts` behind 5 unit tests; the CSS-transform trap (a transform does not change the layout box, so a quarter-turned receipt gets clipped) is handled by measuring the frame and capping the image against the swapped axis.

**Card capture is opt-in, per scan.** An unticked box sends no `cards` field at all, and the route drops `payments` outright unless it is `yes` — the last-4-only enforcement stays, but the stronger guarantee is not reading it back at all. Wording sits with the choice, not in a policy page: only the last four digits, never the full number, nothing saved until Save to Budget, photo deleted when the scan finishes.

**Held-back changelog entry** (this tool is owner-only — publish the day it un-gates, dated then):
> `feature` · **The Receipt Scanner now reads the whole receipt** — Every line item in the order it was printed, each tax line under its own label, and the reference codes along the bottom: receipt number, terminal, TC#, approval code, whatever your shop prints. It checks its own arithmetic and says so, and you can download the lot as a CSV. Before it reads anything you now see the photo, turned the right way up — a receipt photographed on its side was the single biggest cause of a wrong reading. Reading how you paid is a box you tick per scan, off by default, and only ever the last four digits.
