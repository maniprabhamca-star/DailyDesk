...
| REG-035 | | **Every feature API swallowed its own 401.** Notes, Habits, Budget and Link-in-Bio each detected an expired token, showed a polite message about *that feature*, and left the dead token in place — so the header kept claiming you were signed in and the next feature failed the same way. All four now report through `lib/session.ts`, and the auth provider signs you out once, everywhere. | `account.spec.ts` REG-034 covers the shared path | E2E | 🟢 |
| REG-036 | | **A token was never renewed by ordinary use.** `/api/user/me` mints a fresh one, but the front end only called it on `/account` and after a Stripe upgrade — so daily use of the site never extended the session and you were signed out on day 8 regardless. The provider now touches `/me` once per tab, making the window genuinely sliding and surfacing a dead session on arrival. Backend default also raised 7d → 30d. | manual + REG-034 guards the expiry UX | — | 🟢 |
| REG-037 | | **A live Google OAuth client secret sat untracked-but-not-ignored in the repo root**, one `git add -A` from a public commit — and `git add -A` really did stage thousands of files from sixteen stray `.next-*` build directories earlier the same day. Both now in `.gitignore`. Verified the secret was never committed. | `git log --all -- '*ClientID-Secret*'` returns nothing | — | 🟢 |
| REG-038 | | **The header overflowed 29px at 375px — but only when SIGNED IN.** The signed-in header carries an avatar the signed-out one does not. XC-005 checks 375/768/1280 across the archetypes and saw nothing, **because the entire E2E suite browses signed out** — a responsive check that never authenticates is testing half the header. Wordmark now hides below 400px. | `account.spec.ts` → REG-038 ×3 routes, authenticated | E2E | 🟢 |
| REG-039 | | **No way to export your data (GDPR Art. 20) and no way to delete your account (Art. 17)**, while serving the UK and EU. Guidance is blunt that a SaaS which cannot find, export or delete a user is not compliant. Both now exist: one JSON file on demand, and an immediate cascading delete behind email + password re-verification. | `account.spec.ts` → *both GDPR exits are present*, *deleting needs the email typed* | E2E | 🟢 |
| REG-040 | | **The plan card contradicted itself** — an “Active” badge beside “No paid subscription on this account.” Pro without Stripe is real (owner/comped), so the badge reads **Included** and the panel explains why instead of leaving it looking broken. | `account.spec.ts` → *the plan badge does not claim Active with nothing to bill* | E2E | 🟢 |
| REG-041 | | **The home page takes 24.2s to show its h1 under Slow-3G + 4× CPU** (budget 20s). Found by the throttle pass during the full-suite run of 2026-08-08; `/compress-pdf` and `/edit-pdf` both pass the same budget, so this is the home page specifically — it carries the live compressor (pdf.js), the showcase wall, the 102-tool directory and the spotlights. **The budget was NOT raised to make this green**: 24s to first heading on a mid-range phone on a poor connection is a real bounce, and the home page is the entry point for every SEO visitor we are working to attract. | `throttle.spec.ts` → NF-THR `/` (chromium-slow + chromium) | E2E throttle | 🔴 OPEN |
