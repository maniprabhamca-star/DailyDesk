...
const BEYOND: { name: string; desc: string; status: 'live' | 'soon'; tier: 'free' | 'pro' | 'ai' }[] = [
  ...
  { name: 'PDF → EPUB', desc: 'A real e-reader book out of a fixed-layout PDF — chapters, cover, pictures in place, and right-to-left languages laid out properly.', status: 'live', tier: 'free' },
  { name: 'EPUB → PDF / Word', desc: 'The return trip, for when you need the fixed page back.', status: 'soon', tier: 'free' },
  { name: 'HTML → Excel', desc: 'Every table on a web page into a spreadsheet — merged cells handled, which is what copy-paste gets wrong.', status: 'live', tier: 'free' },
  ...
  { name: 'Extract & trim audio', desc: 'Pull the audio out of a video, convert it to MP3 and trim it — no upload.', status: 'soon', tier: 'free' },
